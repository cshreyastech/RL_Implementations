# https://www.youtube.com/watch?v=kjW3Ba4hTYI&list=PL-9x0_FO_lgmP3TtVCD4X1U9oSalSuI1o&index=24
...
class Agent():
  ...
  def learn(self):
    ...
    """
    Have Target and Evaluation network
    Only perform back propagation on the Evaluation network
    and update parameters of target network after certain steps
    """
    ...
    """
    # keepdim=True : so that it does not reduce dimention by 1
    # gather(1) : Gather along action dimention
    # unsqueeze(-1): Unsqueeze along the batch to get a linear array
    # squeeze(1) : squeeze it back to get correct dimention
    
    q_pred = T.add(V_s,
                    (A_s - A_s.mean(dim=1, keepdim=True))).gather(1,
                                    action.unsqueeze(-1)).squeeze(-1)

    q_next = T.add(V_s_,
                    (A_s_ - A_s_.mean(dim=1, keepdim=True)))

    # dim=1 : along action dimention
    # [0] : T.max returns index and values. detach that so that is not
    # involved in back propagation as this is target
